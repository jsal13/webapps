...
function showChord(chord, keyLabel = 'Custom chord') {
  ...
  fretboard.setAttribute('aria-label', `${chord.rootName}${chord.quality} chord tones on a four-string cello fretboard`);
  drawFretboard();
  drawLegend();
}
...
function generateChord() {
  ...
  document.getElementById('fretRangeLabel').textContent = `Open position through fret ${visibleFret}`;
  document.getElementById('fretCount').textContent = `1-${visibleFret}`;
  fretboard.setAttribute('aria-label', `${currentChord.rootName}${currentChord.quality} chord tones on a four-string cello fretboard`);
  drawFretboard();
  drawLegend();
}
